...
(function () {
  ...
  function RiskMgmtCard() {
    return h(QC.Card, { num: "07", title: "Risk management vocabulary",
      ...
      h(Def, { term: "Daily P&L" },
        "Sum of realised + unrealised P&L since UTC midnight. Crypto uses ", h(K, null, "state.daily_pnl"),
        " (freqtrade-internal); stocks doesn't expose a per-day P&L endpoint yet, so the dashboard derives it from ",
  ...
  function ArchitectureCard() {
    return h(QC.Card, { num: "08", title: "Architecture · 5-layer stack",
      sub: "freqtrade ⇄ postgres ⇄ dashboard ⇄ MCP ⇄ Hermes gateway" },
      ...
      h(Def, { term: "freqtrade · 127.0.0.1:8080" },
        "Strategy execution engine. Holds the FreqAI model, manages crypto orders, exposes the ",
        h(K, null, "/api/v1/*"), " REST API. Container ", h(K, null, "freqtrade"), "."),
      h(Def, { term: "postgres · 127.0.0.1:5434" },
        "Long-term storage for trade journal, regime history, sentiment scores, anchors. Container ", h(K, null, "tradebot-postgres"),
        ". TimescaleDB extension for time-series queries."),
      h(Def, { term: "dashboard · 127.0.0.1:8081" },
        "FastAPI app serving the SPA. Reads from freqtrade + postgres + Alpaca + on-chain providers. Container ",
  ...
  function OperationsCard() {
    return h(QC.Card, { num: "09", title: "Operator actions · what each button does",
      ...
      h(Def, { term: "PAUSE TRADING" },
        ...
        " in freqtrade. Existing positions stay open; no new entries fire. Reversible."),
      h(Def, { term: "RESUME" },
      ...
      h(Def, { term: "REBALANCE WEIGHTS" },
        ...
        "Saves to ", h(K, null, "capital_weights.json"), " which freqtrade hot-reloads."),
  ...
  function GlossaryCard() {
    return h(QC.Card, { num: "10", title: "Glossary · A–Z",
      ...
      h("div", { style: { columnCount: 2, columnGap: "var(--s-5)" } },
        ...
        h(Def, { term: "FreqAI" },
          "Freqtrade's built-in ML predictor (gradient boosting). Outputs ", h(K, null, "do_predict"), " + ", h(K, null, "up_prob"), "."),
        h(Def, { term: "Gate" },
