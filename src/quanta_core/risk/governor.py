"""
Pre-trade risk governor (port of ``user_data/modules/risk_governor.py``).

Single point of approval for every entry the strategy is about to send.
Reads its limits from a :class:`RiskConfig` dataclass so a runtime config
reload (without rebuilding the engine) is enough to relax/tighten any
constraint.

Hard gates (any failing → block trade):

1. Portfolio drawdown ≥ ``max_portfolio_drawdown_pct`` → trading paused;
   manual /api/ops/resume only (P0-H — auto-resume removed).
2. Today's realised+unrealised PnL ≤ ``-daily_loss_limit_pct`` of starting
   equity → blocked until next UTC midnight (P0-I — unrealised included).
...
Anchor persistence (P0-G fix from 2026-05-12):
    Daily anchor + DD-pause flag persist to a JSON state file at
    ``~/.quanta/state/risk_governor_anchors.json`` so process restarts
    mid-loss don't reset the loss budget. Backtest-class runmodes
    (``backtest`` / ``hyperopt`` / ``edge``) use a per-process transient
    anchor under ``/tmp`` so simulator runs don't pollute live state
    (Bug 2 from 2026-05-12).

The dedup fix in :meth:`RiskGovernor._pearson_returns` (Bug 1, 2026-05-12)
is preserved verbatim: returns Series with non-unique timestamps are
collapsed via ``~index.duplicated(keep="last")`` BEFORE the join, so the
correlation gate never trips ``pandas.errors.InvalidIndexError``.
"""
...
# ---------------------------------------------------------------------------
# Anchor persistence — runmode-aware, env-overridable.
# ---------------------------------------------------------------------------

#: Default location of the persistent anchor file (live + dry-run).
#: Architecture lock 2026-05-12: state files live under ``~/.quanta/state/``.
_DEFAULT_ANCHOR_PATH = (
    Path(os.path.expanduser("~")) / ".quanta" / "state" / "risk_governor_anchors.json"
)

#: Freqtrade-style runmodes that simulate a historical book. These MUST
#: NOT touch the live anchor file (Bug 2 — 2026-05-12).
...
def _resolve_anchor_path(runmode: str | None = None) -> Path:
    ...
    Priority:
        ...
        3. ``~/.quanta/state/risk_governor_anchors.json`` (default; live + dry).
    """
...
# Back-compat shim: existing callers may call ``_anchor_path()`` with no
# arguments. Route to the resolver with ``runmode=None`` so live behaviour
# is unchanged.
...
@dataclass
class RiskConfig:
    """All operator-tunable risk limits in one dataclass.

    Mirrors the JSON shape of ``config.json[risk_management]`` from the
    Freqtrade-era bot so config files migrate verbatim.
    """
    ...
    def from_dict(cls, d: Mapping[str, Any] | None) -> RiskConfig:
        """Build a config from a (possibly partial) mapping. Unknown keys
        are ignored, so the dataclass can be evolved without breaking the
        on-disk JSON."""
        if not d:
            return cls()
...
@dataclass
class RiskDecision:
    """Result of a single :meth:`RiskGovernor.approve_entry` call."""
    ...
    def outcome(self) -> Outcome:
        """Three-state outcome label used by callers that prefer a tag over
        a boolean. ``warn`` is reserved for downstream (Monte Carlo) gates;
        the governor itself only emits ``pass`` and ``block``.
        """
        return "pass" if self.approved else "block"
...
@dataclass
class TradeRecord:
    """One closed trade. The governor keeps a bounded deque of these for
    Kelly + circuit-breaker accounting."""
...
class RiskGovernor:
    ...
    ----------
    config :
        Risk limits. Defaults are conservative; override per environment.
    now_fn :
        Optional clock injection for testing. Returns a tz-aware UTC datetime.
    runmode :
        Freqtrade-style runmode tag. ``backtest`` / ``hyperopt`` / ``edge``
        route the anchor file to a transient ``/tmp`` path; anything else
        (including ``None``) uses the persistent default.
    """
    ...
    def _persist_anchors(self) -> None:
        ...
        payload = {
            "day_anchor_utc": (self._day_anchor_utc.isoformat() if self._day_anchor_utc else None),
    ...
    def resume_after_manual_review(self, reason: str = "manual_resume") -> bool:
        ...
        ``/api/ops/resume`` (or an MCP tool) after a human has reviewed
        the drawdown event.
        """
    ...
    def from_config(cls, config: Mapping[str, Any]) -> RiskGovernor:
        """Build a governor from a freqtrade-style ``config.json`` dict.

        Extracts ``risk_management`` and ``runmode`` (the latter may be a
        ``freqtrade.enums.RunMode`` enum or a plain string).
        """
    ...
    def update_equity(self, equity: float) -> None:
        ...
        P0-H: auto-resume removed. Once paused, only
        :meth:`resume_after_manual_review` clears the flag.
        """
        ...
        if self._day_anchor_utc is None or now.date() > self._day_anchor_utc.date():
            ...
            logger.info(
                ...
                self._day_anchor_utc.date().isoformat(),
                equity,
            )

        # Drawdown pause — trip-only. No auto-resume; operator inspects.
    ...
    def record_trade_close(
        ...
        """Record a closed trade. Updates daily PnL + Kelly + circuit breaker."""
        ...
        if pnl_quote < 0:
            ...
            if (
                ...
                self._cooldown_until = ts + timedelta(
                    hours=self.config.circuit_breaker_cooldown_hours
                )
                logger.warning(
                    "[risk] CIRCUIT BREAKER — %d consecutive losses → cooldown until %s",
    ...
    def approve_entry(
        ...
        pair :
            e.g. ``"BTC/USD"``.
        signal_price :
            Model's reference price (used by the execution layer for
            slippage checks; not consumed by the governor itself).
        base_stake :
            Proposed stake in quote currency before risk sizing.
        equity :
            Current portfolio equity in quote currency.
        model_confidence :
            Meta-agent confidence in ``[0, 1]``; required if Kelly enabled.
        open_positions :
            Iterable of ``(pair, stake_in_quote)`` currently open.
        pair_returns :
            Dict mapping pair → return series; the candidate pair must
            be present along with each open-position pair, otherwise the
            correlation gate is skipped (and logged).
        open_unrealised_pnl :
            Signed mark-to-market P&L of currently open positions in
            quote currency. P0-I: included in the daily-loss check so
            the bot can't keep opening trades while sitting on a big
            paper loss that the realised-only number misses.
        """
    ...
    def _kelly_fraction(self, confidence: float) -> float:
        ...
        ``f* = (p·b − q) / b`` with safety scaling, where ``p`` is the
        model's win probability, ``q = 1 − p`` and ``b = avg_win /
        avg_loss`` from the last ``kelly_lookback_trades`` closed trades.

        Returns ``0.0`` when fewer than ``kelly_min_trades`` closed trades
        are on record — the caller will fall back to ``base_stake`` capped
        by ``max_position_size_pct``.
        """
        cfg = self.config
    ...
    def _pearson_returns(self, a: pd.Series, b: pd.Series) -> float | None:
        ...
        if not isinstance(a.index, pd.DatetimeIndex) or not isinstance(b.index, pd.DatetimeIndex):
            ...
            ax_series = a[a.index >= cutoff]
            bx_series = b[b.index >= cutoff]
            joined = pd.concat([ax_series, bx_series], axis=1, join="inner").dropna()
    ...
    def status(self) -> dict[str, Any]:
        """Operator-friendly snapshot for ``/api/ops/risk`` endpoints."""
        now = self._now()
        return {
            ...
            "peak_equity": self._peak_equity,
            "drawdown_pct": (
                0.0 if self._peak_equity == 0 else 1.0 - self._current_equity / self._peak_equity
            ),
            "paused_for_drawdown": self._paused_for_drawdown,
            "daily_realized_pnl": self._daily_realized_pnl,
            "starting_equity_today": self._starting_equity_today,
